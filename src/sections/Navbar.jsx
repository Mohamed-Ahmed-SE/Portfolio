...
const NavbarLink = ({ item, toggleMenu }) => {
      ...
      >
        <div className="flex flex-col text-5xl gap-y-2 md:text-6xl lg:text-8xl">
          {[
            ...
            { name: "work", path: "/projects", type: "route" },
            { name: "contact", path: "/contact", type: "route" },
          ].map((item, index) => (
            <div key={index} ref={(el) => (linksRef.current[index] = el)}>
              {/* Logic: If route, use useNavigate/Link. If scroll, check if on home, then scroll, else nav to home#id */}
              <NavbarLink item={item} toggleMenu={toggleMenu} />
            </div>
          ))}
        </div>
        <div
